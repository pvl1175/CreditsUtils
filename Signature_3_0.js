function ConstructTransaction(ApiConnect, Obj,Transac){
    ...
    //amount: "10.1",
    //fee: "1.1",
    //source: Base58.decode("5B3YXqDTcWQFGAqEJQJP3Bg1ZK8FFtHtgCiFLT5VAxpe"),
    //Priv: Base58.decode("3rUevsW5xfob6qDxWMDFwwTQCq39SYhzstuyfUGSDvF2QHBRyPD8fSk49wFXaPk3GztfxtuU85QHfMV3ozfqa7rN"),
    //target: Base58.decode("4tEQbQPYZq1bZ8Tn9DpCXYUgPgEgcqsBPXX4fXef7FuL"),
    //smart: {
    //    method: "",
    //    params: [{key,value}]
    ...
    if (SourseId.status.message === "Success") {
    ...
    if (Obj.smart !== undefined && Obj.smart.code !== undefined) {
        ...
        console.log(BytesC.join(" "));
        let ByteCodeS = ApiConnect.SmartContractCompile(Obj.smart.code).byteCode;

        BytesC = concatTypedArrays(BytesC, new Uint8Array(ByteCodeS.length));
        for (i in ByteCodeS) {
            BytesC[Number(i) + 38] = ByteCodeS[i].charCodeAt();
        }
        ...
        if (TargetId.status.message === "Success") {
    ...
    let F = (Number(Obj.amount) - Math.trunc(Obj.amount)) * Math.pow(10, 18);
    ...
    var InnerId = NumbToByte(Trans.amount.fraction, 8);
    ...
    if (Obj.smart !== undefined) {
        ...
        if (Obj.smart.params === undefined) {
            ...
            for (let i in Obj.smart.params) {
                switch (Obj.smart.params[i].key) {
                    case "STRING":
                        Bytes = concatTypedArrays(Bytes, new Uint8Array([11, 0, 7]));
                    ...
                    case "I64":
                        ...
                        var InnerId = NumbToByte(Obj.smart.params[i].value, 8);
        ...
        Bytes = new Uint8Array([11, 0, 2]);
        if (Obj.smart.code == undefined && Obj.smart.code == null) {
            Bytes = concatTypedArrays(Bytes, new Uint8Array(4));
        } else {
            Trans.smartContract.smartContractDeploy.byteCode = ApiConnect.SmartContractCompile(Obj.smart.code).byteCode;
            var ByteCode = ApiConnect.SmartContractCompile(Obj.smart.code).byteCode;


            Bytes = concatTypedArrays(Bytes, GetBitArray(ByteCode.length, 4).reverse());

            Bytes = concatTypedArrays(Bytes, new Uint8Array(ByteCode.length));

            for (i in ByteCode) {
                Bytes[Number(i) + 7] = ByteCode[i].charCodeAt();
            }
        }
